...
extension LinearBoundedOutputSpanTests.Unit {
    ...
    func `init with partial population leaves correct count`() throws {
        let buffer = Buffer<Storage<Memory.Allocator<Memory.Heap>>.Contiguous<Int>>.Linear.Bounded(capacity: 8) { span in
            span.append(1)
            span.append(2)
            span.append(3)
        }
...
extension LinearBoundedOutputSpanTests.EdgeCase {
    ...
    func `OutputSpan freeCapacity decreases with appends`() throws {
        var captured: [Int] = []
        let buffer = Buffer<Storage<Memory.Allocator<Memory.Heap>>.Contiguous<Int>>.Linear.Bounded(capacity: 3) { span in
    ...
    func `OutputSpan isFull reflects requested capacity`() throws {
        ...
        let buffer = Buffer<Storage<Memory.Allocator<Memory.Heap>>.Contiguous<Int>>.Linear.Bounded(capacity: 2) { span in
            span.append(1)
            span.append(2)
...
extension LinearBoundedOutputSpanTests.NonCopyable {
    ...
    func `init with noncopyable elements`() throws {
        let buffer = Buffer<Storage<Memory.Allocator<Memory.Heap>>.Contiguous<MoveOnly>>.Linear.Bounded(capacity: 3) { span in
    ...
    func `init partial-populate with noncopyable elements`() throws {
        let buffer = Buffer<Storage<Memory.Allocator<Memory.Heap>>.Contiguous<MoveOnly>>.Linear.Bounded(capacity: 5) { span in
...
extension LinearBoundedOutputSpanTests.Throwing {
    ...
    func `init throws propagates the error`() {
        #expect(throws: FixtureError.deliberate) {
            _ = try Buffer<Storage<Memory.Allocator<Memory.Heap>>.Contiguous<Int>>.Linear.Bounded(capacity: 4) { span throws(FixtureError) in
                span.append(1)
                span.append(2)
    ...
    func `init throws with noncopyable elements — elements cleaned up`() {
        ...
        #expect(throws: FixtureError.deliberate) {
            _ = try Buffer<Storage<Memory.Allocator<Memory.Heap>>.Contiguous<MoveOnly>>.Linear.Bounded(capacity: 3) { span throws(FixtureError) in
                span.append(MoveOnly(1))
                span.append(MoveOnly(2))
                throw FixtureError.deliberate
            }
