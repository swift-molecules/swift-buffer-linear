...
extension LinearOutputSpanTests.Append {
    ...
    func `append adds to existing buffer`() throws {
        ...
        buffer.append(addingCapacity: .init(_unchecked: Cardinal(3))) { span in
            span.append(10)
            span.append(20)
            span.append(30)
        }
        #expect(buffer.count == .init(_unchecked: Cardinal(5)))
    }
    ...
    func `append triggers growth when required exceeds capacity`() throws {
        ...
        buffer.append(addingCapacity: .init(_unchecked: Cardinal(10))) { span in
            for i in 0..<10 {
                span.append(100 + i)
            }
        }
        #expect(buffer.count == .init(_unchecked: Cardinal(12)))
        #expect(buffer.capacity >= .init(_unchecked: Cardinal(12)))
    ...
    func `append with partial population commits what was added`() throws {
        ...
        buffer.append(addingCapacity: .init(_unchecked: Cardinal(5))) { span in
            span.append(10)
            span.append(20)
    ...
    func `append with zero addingCapacity is a noop`() throws {
        ...
        buffer.append(addingCapacity: .init(_unchecked: Cardinal(0))) { span in
...
extension LinearOutputSpanTests.Edit {
    ...
    func `edit can append beyond current count up to capacity`() throws {
        ...
        buffer.reserveCapacity(.init(_unchecked: Cardinal(10)))
...
extension LinearOutputSpanTests.NonCopyable {
    ...
    func `append with noncopyable elements, triggering growth`() throws {
        ...
        buffer.append(addingCapacity: .init(_unchecked: Cardinal(4))) { span in
...
extension LinearOutputSpanTests.Throwing {
    ...
    func `append throw preserves already-initialized elements`() throws {
        ...
        do {
            try buffer.append(addingCapacity: .init(_unchecked: Cardinal(5))) { span throws(FixtureError) in
                span.append(1)
                span.append(2)
                throw FixtureError.deliberate
            }
